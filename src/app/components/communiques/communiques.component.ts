...
@Component({
    ...
    styleUrls: ['./communiques.component.scss', '../liste-actualites/liste-actualites.component.scss'],
...
export class CommuniquesComponent implements OnInit {
    ...
    contentData: any;
    category: any;
    itemsPerPage: number = NOMBRE_AFFICHAGE_COMMUNIQUES;
    currentPage: number;
    titlePage: any;
    ...
    ngOnInit() {
        this._data.dataResponse.subscribe(art => {
            if (art.code === codeWS.PAGE_LISTING_COMMUNIQUES) {
                this.contentData = art.data.data.posts;
                this.itemsCount = art.data.data.count;
                this.itemsPerPage = NOMBRE_AFFICHAGE_COMMUNIQUES;
    ...
    goTopage(_pageId: string) {
        if (this.currentIdCategory =="") this._data.getWpData('/bfm/communiques?page=' + _pageId);
        else this._data.getWpData('/bfm/communiques?page=' + _pageId + '&idcategorie=' + this.currentIdCategory);
    }

    pageChanged(event: any): void {
        if (this.currentIdCategory != ""){
            this.router.navigate(['/communique/categorie/' + this.currentIdCategory + '/page' + event.page]);
        } else{
            this.router.navigate(['/communique/page', event.page]);
        }
    }
    ...
    filter_by_cat(filterVal: any, currentPage: number) {
        if (filterVal == 0) {
            this.router.navigate(['/communique/page/'+ this.reinitpagination]);
        }
        else {
            this.router.navigate(['/communique/categorie/'+ filterVal + '/page/' + this.reinitpagination]);
        }
    }
}
